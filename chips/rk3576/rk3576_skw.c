 ...
 *   the command waiter; events are handled separately (TODO: scan/connect
 *   events for the netdev layer).
 *
 ****************************************************************************/
                        ...
                        uint8_t *resp, int *resp_len)
{
  ...
  if (msg_len > SKW_CMD_MAXLEN || padded > (int)sizeof(g_skw_txbuf))
    {
      return -E2BIG;
    }
  ...
  g_skw_txbuf[4] = (SKW_MSG_CMD << 4) | 0;      /* inst 0, type CMD */
  g_skw_txbuf[5] = id;
  g_skw_txbuf[6] = g_skw_cmd.seq & 0xff;
  g_skw_txbuf[7] = (g_skw_cmd.seq >> 8) & 0xff;
  g_skw_txbuf[8] = msg_len & 0xff;
  g_skw_txbuf[9] = (msg_len >> 8) & 0xff;

  if (plen > 0)
    {
      memcpy(g_skw_txbuf + 12, payload, plen);
    }

  eof = SKW_HDR_EOF_TERM;
  g_skw_txbuf[4 + msg_pad + 0] = eof & 0xff;
  g_skw_txbuf[4 + msg_pad + 1] = (eof >> 8) & 0xff;
