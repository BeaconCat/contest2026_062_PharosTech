/****************************************************************************
 * vendor/rockchip/chips/rk3576/hardware/rk3576_vop.h
 *
...
#define RK3576_VOP_ESMART(n)  (RK3576_VOP_ESMART_BASE + \
                               (n) * RK3576_VOP_ESMART_STRIDE)
#define RK3576_VOP_CLUSTER(n) (RK3576_VOP_CLUSTER_BASE + \
                               (n) * RK3576_VOP_CLUSTER_STRIDE)
...
#define VOP_DSP_IF_EN_HDMI0_MUX(vp)   (((uint32_t)(vp) << \
                                        VOP_DSP_IF_EN_HDMI0_MUX_SHIFT) & \
...
#define VOP_OVL_LAYER_SEL(layer, win)     (((uint32_t)(win) & 0xfu) << \
                                           VOP_OVL_LAYER_SEL_SHIFT(layer))
...
#define VOP_OVL_PORT_SEL_NLAYER_MASK(vp)  (0xfu << VOP_OVL_PORT_SEL_NLAYER_SHIFT(vp))
#define VOP_OVL_PORT_SEL_NLAYER(vp, n)    (((uint32_t)(n) & 0xfu) << \
                                           VOP_OVL_PORT_SEL_NLAYER_SHIFT(vp))

#define VOP_OVL_PORT_SEL_LAYER_SHIFT(layer) (16 + (layer) * 2)
#define VOP_OVL_PORT_SEL_LAYER_MASK(layer)  (3u << VOP_OVL_PORT_SEL_LAYER_SHIFT(layer))
#define VOP_OVL_PORT_SEL_LAYER(layer, vp)   (((uint32_t)(vp) & 3u) << \
                                             VOP_OVL_PORT_SEL_LAYER_SHIFT(layer))
...
#define VOP_TIMING_PACK(hi, lo)  ((((uint32_t)(hi) & 0xffffu) << 16) | \
                                  ((uint32_t)(lo) & 0xffffu))
...
#define VOP_ESMART_VIR_STRIDE(words)       ((uint32_t)(words) & \
                                            VOP_ESMART_VIR_STRIDE_MASK)
...
#define VOP_WIN_SIZE_PACK(w, h)  ((((uint32_t)(h) - 1u) << 16) | \
                                  (((uint32_t)(w) - 1u) & 0xffffu))
#define VOP_WIN_POS_PACK(x, y)   ((((uint32_t)(y) & 0xffffu) << 16) | \
                                  ((uint32_t)(x) & 0xffffu))
