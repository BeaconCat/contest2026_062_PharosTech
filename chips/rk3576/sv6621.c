 ...
 * no GPL driver code is copied.  Host-controller details are supplied
 * through struct sv6621_transport_s, keeping the CP protocol, firmware,
 * WPA2 and network paths independent of the platform SDIO implementation.
 *
...
#define SKW_REG_CP2AP_FIFO \
  0x181 /* CP RX FIFO non-empty indication (poll data-ready) */
...
#define SKW_HDR(ch, len) (((uint32_t)(ch) << 24) | ((uint32_t)(len)&0xffff))
...
/* sv6621_msg (8-byte command header). */
...
/* WiFi events (sv6621_msg type=EVENT). */
 ...
 * real content (sv6621_msg / ethernet frame / loopcheck text).
 */
...
static const struct sv6621_config_s *g_sv6621_config;
static uint32_t g_sv6621_service;
static bool g_sv6621_run;

/* Last CP2AP FIFO level snapshot, read by the rx thread to gate work. */

static volatile uint8_t g_sv6621_fifo_last;

static uint8_t g_sv6621_mac[6];
static uint8_t g_sv6621_bssid[6];
static uint8_t g_sv6621_peer_idx;
static uint8_t g_sv6621_lmac;
static uint8_t g_sv6621_mcast;

/* Scan result cache (SSID/BSSID/channel/rssi), filled from SCAN_REPORT
 * events, drained by sv6621_scan().
 */

struct sv6621_bss_entry_s
{
  uint8_t bssid[6];
  uint8_t ssid[33];
  uint8_t ssid_len;
...
static struct sv6621_bss_entry_s g_sv6621_scan[SKW_MAX_SCAN];
static int g_sv6621_scan_n;
static volatile bool g_sv6621_scanning;
static volatile bool g_sv6621_scan_done;

/* Connection state driven by JOIN/ASOCC/DISCONNECT events. */

static volatile int g_sv6621_conn_evt; /* last connect event id, -1 idle */

/* RX burst scratch (0x600*n + 512, one 100-packet burst worst case). */

static uint8_t g_sv6621_rxbuf[SKW_PAC_SIZE * 8 + 512] aligned_data(64);

/* TX packet scratch (header + msg + payload, block-padded). */

static uint8_t g_sv6621_cmdbuf[2048] aligned_data(64);
static uint8_t g_sv6621_txbuf[2048] aligned_data(64);
static mutex_t g_sv6621_cmd_lock = NXMUTEX_INITIALIZER;
static mutex_t g_sv6621_tx_lock = NXMUTEX_INITIALIZER;
...
static int sv6621_cmd52(bool write, uint32_t function, uint32_t address,
                        uint8_t value, FAR uint8_t *result);
static void sv6621_dt_latch(uint32_t cp_address);
static int sv6621_cmd53_read(uint32_t function, uint32_t address,
                             bool increment, FAR uint8_t *buffer, int length);
static int sv6621_cmd53_write(uint32_t function, uint32_t address,
                              bool increment, FAR const uint8_t *buffer,
                              int length);
static int sv6621_dt_stream(uint32_t cp_address, FAR const uint8_t *buffer,
                            int length);
static void sv6621_handle_loopcheck(FAR const uint8_t *payload, int length);
static void sv6621_handle_event(uint8_t id, FAR const uint8_t *event,
                                int length);
static void sv6621_handle_wifi_cmd(FAR const uint8_t *payload, int length);
static void sv6621_data_rx(FAR const uint8_t *payload, int length);
static int sv6621_rx_burst(int buffer_count);
static int sv6621_rx_thread(int argc, FAR char **argv);
static int sv6621_send_cmd(uint8_t id, FAR const uint8_t *payload, int length,
                           FAR uint8_t *response, FAR int *response_length);
static int sv6621_wifi_bringup_cmds(void);
static int sv6621_scan_request(void);
static FAR uint8_t *sv6621_nv_patch(FAR const uint8_t *iram, int iram_length);
static int sv6621_download_and_boot(void);
static int sv6621_bringup(void);
static int sv6621_wait_conn_evt(int timeout_ms);
static bool sv6621_bss_is_rsn(FAR const struct sv6621_bss_entry_s *bss);
static int sv6621_associate(FAR const struct sv6621_bss_entry_s *bss);
...
{
  FAR struct sv6621_transport_s *transport = g_sv6621_config->transport;

  if (write)
    {
      return transport->ops->writeb(transport, function, address, value);
    }

  return transport->ops->readb(transport, function, address, result);
}
/****************************************************************************
 * Name: sv6621_dt_latch
 *
 * Description:
...
static void sv6621_dt_latch(uint32_t cpaddr)
{
  sv6621_cmd52(true, 0, SKW_FBR_ADDR + 0, cpaddr & 0xff, NULL);
  sv6621_cmd52(true, 0, SKW_FBR_ADDR + 1, (cpaddr >> 8) & 0xff, NULL);
  sv6621_cmd52(true, 0, SKW_FBR_ADDR + 2, (cpaddr >> 16) & 0xff, NULL);
  sv6621_cmd52(true, 0, SKW_FBR_ADDR + 3, (cpaddr >> 24) & 0xff, NULL);
}

/****************************************************************************
 * Name: sv6621_cmd53_read / sv6621_cmd53_write
 *
 * Description:
 *   Forward an SDIO extended transfer to the platform transport.  The
 *   transport selects byte or block mode and owns all host-controller
 *   details.
 *
 ****************************************************************************/
                             ...
                             bool increment, uint8_t *buffer, int length)
{
  ...
  return transport->ops->read(transport, function, address, increment, buffer,
                              length);
}

static int sv6621_cmd53_write(uint32_t function, uint32_t address,
                              bool increment, const uint8_t *buffer,
                              int length)
{
  FAR struct sv6621_transport_s *transport = g_sv6621_config->transport;

  return transport->ops->write(transport, function, address, increment, buffer,
                               length);
}
/****************************************************************************
 * Name: sv6621_dt_stream
 *
 * Description:
...
static int sv6621_dt_stream(uint32_t cpaddr, const uint8_t *buf, int len)
{
  int off = 0;

  sv6621_dt_latch(cpaddr);
  ...
  while (off < len)
    {
      ...
      ret = sv6621_cmd53_write(1, SKW_DT_WINDOW, true, buf + off, chunk);
      for (try = 0; ret < 0 && try < 4; try++)
        {
          sv6621_cmd52(true, 0, 0x06, 0x01, NULL); /* I/O abort */
          up_mdelay(2);
          sv6621_dt_latch(cpaddr + off);
          ret = sv6621_cmd53_write(1, SKW_DT_WINDOW, true, buf + off, chunk);
        }
...
static void sv6621_handle_loopcheck(const uint8_t *pl, int len)
{
...

  {
    ...
    txt[n] = 0;
    syslog(LOG_ERR, "SV6621: cp> %s\n", txt);
  }

  if (memmem(pl, len, "trunk_W", 7) != NULL &&
      !(g_sv6621_service & SV6621_STATE_BSP))
    {
      g_sv6621_service |= SV6621_STATE_BSP;

      /* WIFI_START doorbell: 1 << ((service 0 << 1) | cmd 0). */

      sv6621_cmd52(true, 0, SKW_REG_AP2CP_IRQ, 0x01, NULL);
    }

  if (memmem(pl, len, "WIFIREADY", 9) != NULL)
    {
      g_sv6621_service |= SV6621_STATE_WIFI;
    }

  if (memmem(pl, len, "BTREADY", 7) != NULL)
    {
      g_sv6621_service |= SV6621_STATE_BT;
    }
}

/****************************************************************************
 * Name: sv6621_handle_wifi_cmd
 *
 * Description:
 *   Dispatch a channel-6 packet: an sv6621_msg (8-byte header) followed by
...
/****************************************************************************
 * Name: sv6621_handle_event
 *
 * Description:
 *   Handle a WiFi event (sv6621_msg type=EVENT).  SCAN_REPORT carries an
 *   sv6621_mgmt_hdr {chan, band, s16 signal, u16 mgmt_len, u16 resv} followed
...
static void sv6621_handle_event(uint8_t id, const uint8_t *ev, int len)
{
  if (id == SKW_EVENT_SCAN_CMPL)
    {
      g_sv6621_scan_done = true;
      return;
    }

  if (id == SKW_EVENT_ASOCC || id == SKW_EVENT_JOIN_CMPL)
    {
      g_sv6621_conn_evt = SKW_CONN_ASSOC_OK;
      return;
    }
  ...
  if (id == SKW_EVENT_DISCONNECT || id == SKW_EVENT_DEAUTH ||
    ...
    {
      g_sv6621_conn_evt = SKW_CONN_ASSOC_FAIL;
      return;
    }
   ...
   * sv6621_mgmt_hdr {chan, band, s16 signal, u16 mgmt_len, u16 resv} + frame.
      ...
      if (subtype == 0x10) /* assoc-response */
        {
          ...
          syslog(LOG_INFO, "SV6621: assoc-resp status=%u\n", status);
          g_sv6621_conn_evt =
              (status == 0) ? SKW_CONN_ASSOC_OK : SKW_CONN_ASSOC_FAIL;
        }
  ...
  if (id == SKW_EVENT_SCAN_REPORT && len >= 8 + 24)
    {
      ...
      if (mlen < 36 || g_sv6621_scan_n >= SKW_MAX_SCAN)
        {
          return;
        }
      ...
      bss = &g_sv6621_scan[g_sv6621_scan_n];
      memset(bss, 0, sizeof(*bss));
      memcpy(bss->bssid, mgmt + 16, 6);
      bss->channel = chan;
      bss->band = ev[1]; /* band from sv6621_mgmt_hdr */
      bss->rssi = signal;
      ...
      bss->ie_len =
          (mlen - 36 > (int)sizeof(bss->ie)) ? sizeof(bss->ie) : (mlen - 36);
      ...
      for (i = 0; i < g_sv6621_scan_n; i++)
        {
          if (memcmp(g_sv6621_scan[i].bssid, bss->bssid, 6) == 0)
            {
              return;
            }
        }
...
static void sv6621_handle_wifi_cmd(const uint8_t *pl, int len)
{
  const uint8_t *msg;
  int mlen;
  uint8_t type;
  ...
  /* Skip the 12-byte inner link header; the sv6621_msg follows. */
  ...
  if (type == SKW_MSG_EVENT)
    {
      sv6621_handle_event(id, msg + 8, mlen - 8);
      return;
    }

  if (type == SKW_MSG_CMD_ACK && g_sv6621_cmd.waiting &&
      id == g_sv6621_cmd.id && seq == g_sv6621_cmd.seq)
    {
      /* ACK: sv6621_msg(8) + status(u16) + return data. */

      g_sv6621_cmd.status = (mlen >= 10) ? (msg[8] | (msg[9] << 8)) : -1;
      g_sv6621_cmd.resp_len = (mlen > 10) ? (mlen - 10) : 0;
      if (g_sv6621_cmd.resp_len > (int)sizeof(g_sv6621_cmd.resp))
        {
          g_sv6621_cmd.resp_len = sizeof(g_sv6621_cmd.resp);
        }

      if (g_sv6621_cmd.resp_len > 0)
        {
          memcpy(g_sv6621_cmd.resp, msg + 10, g_sv6621_cmd.resp_len);
        }

      g_sv6621_cmd.waiting = false;
      nxsem_post(&g_sv6621_cmd.done);
    }
}
...
static void sv6621_data_rx(const uint8_t *pl, int len)
{
  int i;
      ...
      if (pl[i] == 0x88 && pl[i + 1] == 0x8e)
        {
          ...
          if (ethlen > 14)
            {
              sv6621_wpa_eapol_input(eth + 14, ethlen - 14);
            }
...
static int sv6621_rx_burst(int buf_num)
{
  int readlen;
  uint32_t rx_nsize;
  int off;
  ...
  ret = sv6621_cmd53_read(1, SKW_PK_WINDOW, false, g_sv6621_rxbuf, readlen);
  if (ret < 0)
    {
      return ret;
    }
  ...
  for (off = 0; off + 4 <= SKW_PAC_SIZE * buf_num; off += SKW_PAC_SIZE)
    {
      const uint8_t *p = g_sv6621_rxbuf + off;
      uint32_t hdr =
...
#ifdef CONFIG_SV6621_DEBUG
      if (ch == SKW_CH_WIFI_CMD || ch == SKW_CH_WIFI_DATA)
        {
          syslog(LOG_ERR,
                 "SV6621: rx ch=%" PRIu32 " len=%d off=%d hdr=%08" PRIx32 "\n",
                 ch, plen, off, hdr);
          syslog(LOG_ERR,
                 "SV6621:   %02x %02x %02x %02x %02x %02x %02x %02x "
      ...
      switch (ch)
        {
          case SKW_CH_LOOPCHECK:
            sv6621_handle_loopcheck(p + 4, plen);
            break;

          case SKW_CH_WIFI_CMD:
            sv6621_handle_wifi_cmd(p + 4, plen);
            break;

          case SKW_CH_WIFI_DATA:
            sv6621_data_rx(p + 4, plen);
            break;
  ...
  rx_nsize = g_sv6621_rxbuf[readlen - 4] | (g_sv6621_rxbuf[readlen - 3] << 8) |
             (g_sv6621_rxbuf[readlen - 2] << 16) |
             ((uint32_t)g_sv6621_rxbuf[readlen - 1] << 24);

  return (int)rx_nsize;
}

/****************************************************************************
 * Name: sv6621_rx_thread
 *
 * Description:
...
static int sv6621_rx_thread(int argc, char **argv)
{
  int buf_num = 1;
  ...
  while (g_sv6621_run)
    {
      ...
      sv6621_cmd52(false, 0, SKW_REG_INTX, 0, &intx);

      /* Acknowledge the transport's latched DAT1 indication after sampling
       * the CP status.  Leaving it latched can prevent the CP from
       * re-presenting the packet window.
       */

      g_sv6621_config->transport->ops->ack_interrupt(
          g_sv6621_config->transport);

      bool fifo_changed = false;
      {
        uint8_t fifo_indicator = 0;

        sv6621_cmd52(false, 0, SKW_REG_CP2AP_FIFO, 0, &fifo_indicator);
        if (fifo_indicator != g_sv6621_fifo_last)
          {
            g_sv6621_fifo_last = fifo_indicator;
            fifo_changed = true;
          }
      }
...

      {
        ...
        if (!fifo_changed && !g_sv6621_cmd.waiting)
          {
            idle_cnt++;
            if ((idle_cnt & 7) != 0) /* ~1 in 8 idle polls reads */
              {
                up_mdelay(g_sv6621_scanning ? 2 : 5);
                continue;
              }
          }
      }
      ...
      for (drained = 0; drained < 16 && g_sv6621_run; drained++)
        {
          nsize = sv6621_rx_burst(buf_num);
          if (nsize <= 0)
            {
              uint8_t ext = 0;
              ...
              sv6621_cmd52(false, 0, SKW_REG_INT_EXT, 0, &ext);
              buf_num = 1;
              break;
            }
      ...
      up_mdelay(g_sv6621_scanning ? 5 : 20);
    }
...
static int sv6621_send_cmd(uint8_t id, const uint8_t *payload, int plen,
                           uint8_t *resp, int *resp_len)
{
  ...
  if (msg_len > SKW_CMD_MAXLEN || padded > (int)sizeof(g_sv6621_cmdbuf))
    {
      return -E2BIG;
    }

  ret = nxmutex_lock(&g_sv6621_cmd_lock);
  if (ret < 0)
    {
      return ret;
    }
  ...
  g_sv6621_cmd.seq++;
  g_sv6621_cmd.id = id;
  g_sv6621_cmd.status = -1;
  g_sv6621_cmd.resp_len = 0;
  g_sv6621_cmd.waiting = true;

  memset(g_sv6621_cmdbuf, 0, padded);

  /* Channel-6 data header (eof=0), then the sv6621_msg + payload, then a
   * trailing eof-only terminator header (matches setup_sdio2_packet).
   */

  hdr = SKW_HDR(SKW_CH_WIFI_CMD, msg_len);
  g_sv6621_cmdbuf[0] = hdr & 0xff;
  g_sv6621_cmdbuf[1] = (hdr >> 8) & 0xff;
  g_sv6621_cmdbuf[2] = (hdr >> 16) & 0xff;
  g_sv6621_cmdbuf[3] = (hdr >> 24) & 0xff;

  /* sv6621_msg: inst_id:4|type:4, id, seq(u16), total_len(u16), resv[2]. */

  g_sv6621_cmdbuf[4] = (SKW_MSG_CMD << 4) | 0; /* inst 0, type CMD */
  g_sv6621_cmdbuf[5] = id;
  g_sv6621_cmdbuf[6] = g_sv6621_cmd.seq & 0xff;
  g_sv6621_cmdbuf[7] = (g_sv6621_cmd.seq >> 8) & 0xff;
  g_sv6621_cmdbuf[8] = msg_len & 0xff;
  g_sv6621_cmdbuf[9] = (msg_len >> 8) & 0xff;

  if (plen > 0)
    {
      memcpy(g_sv6621_cmdbuf + 12, payload, plen);
    }

  eof = SKW_HDR_EOF_TERM;
  g_sv6621_cmdbuf[4 + msg_pad + 0] = eof & 0xff;
  g_sv6621_cmdbuf[4 + msg_pad + 1] = (eof >> 8) & 0xff;
  g_sv6621_cmdbuf[4 + msg_pad + 2] = (eof >> 16) & 0xff;
  g_sv6621_cmdbuf[4 + msg_pad + 3] = (eof >> 24) & 0xff;

  ret = nxmutex_lock(&g_sv6621_tx_lock);
  if (ret >= 0)
    {
      ret =
          sv6621_cmd53_write(1, SKW_PK_WINDOW, false, g_sv6621_cmdbuf, padded);
      if (ret >= 0)
        {
          /* Ring the AP->CP doorbell so the CP services the command. */

          ret = sv6621_cmd52(true, 0, SKW_REG_AP2CP_IRQ, 0x01, NULL);
        }

      nxmutex_unlock(&g_sv6621_tx_lock);
    }
#ifdef CONFIG_SV6621_DEBUG
  syslog(LOG_ERR, "SV6621: tx cmd id=%u seq=%u msglen=%d padded=%d wr=%d\n",
         id, g_sv6621_cmd.seq, msg_len, padded, ret);
#endif
  if (ret < 0)
    {
      g_sv6621_cmd.waiting = false;
      nxmutex_unlock(&g_sv6621_cmd_lock);
      return ret;
    }

  ret = nxsem_tickwait(&g_sv6621_cmd.done, MSEC2TICK(SKW_CMD_TIMEOUT_MS));
  if (ret < 0)
    {
      g_sv6621_cmd.waiting = false;
  ...
  if (resp != NULL && resp_len != NULL)
    {
      int n = g_sv6621_cmd.resp_len < *resp_len ? g_sv6621_cmd.resp_len
                                                : *resp_len;

      memcpy(resp, g_sv6621_cmd.resp, n);
      *resp_len = n;
    }

  ret = g_sv6621_cmd.status;
  nxmutex_unlock(&g_sv6621_cmd_lock);
  return ret;
}

/****************************************************************************
 * Name: sv6621_wifi_bringup_cmds
 *
 * Description:
...
static int sv6621_wifi_bringup_cmds(void)
{
  ...
  sv6621_cmd52(true, 0, SKW_REG_CPLOG_SW, 0x01, NULL);
  sv6621_cmd52(true, 0, SKW_REG_AP2CP_IRQ, 1u << 5, NULL);
...

  {
    ...
    ret = sv6621_send_cmd(SKW_CMD_SYN_VERSION, ver, sizeof(ver), NULL, NULL);
    if (ret < 0)
      {
  ...
  ret = sv6621_send_cmd(SKW_CMD_GET_INFO, NULL, 0, resp, &resp_len);
  if (ret < 0)
    {
  ...
  g_sv6621_mac[1] = 0xfd;
  g_sv6621_mac[2] = 0xfc;
  g_sv6621_mac[3] = (resp_len > 0) ? resp[0] : 0x01;
  g_sv6621_mac[4] = (resp_len > 1) ? resp[1] : 0x02;
  g_sv6621_mac[5] = 0x03;

  syslog(LOG_INFO,
         "SV6621: GET_INFO ok (%d bytes) MAC %02x:%02x:%02x:%02x:%02x:%02x\n",
         resp_len, g_sv6621_mac[0], g_sv6621_mac[1], g_sv6621_mac[2],
         g_sv6621_mac[3], g_sv6621_mac[4], g_sv6621_mac[5]);
  ...
  if (g_sv6621_config->calib != NULL && g_sv6621_config->calib_len > 0)
    {
      const uint8_t *cd = g_sv6621_config->calib;
      int remain = g_sv6621_config->calib_len;
      uint8_t chunk[4 + 512];
      int seq = 0;
      ...
      while (remain > 0)
        {
          ...
          ret =
              sv6621_send_cmd(SKW_CMD_PHY_BB_CFG, chunk, 4 + len, NULL, NULL);
          if (ret < 0)
            {
      ...
      syslog(LOG_ERR, "SV6621: calib download %s (%d bytes)\n",
             ret == 0 ? "ok" : "FAILED", g_sv6621_config->calib_len);
    }
...

  {
    ...
    op[2] = 0; /* u16 flags = 0 */
    op[3] = 0;
    memcpy(op + 4, g_sv6621_mac, 6);

    ret = sv6621_send_cmd(SKW_CMD_OPEN_DEV, op, sizeof(op), NULL, NULL);
    if (ret < 0)
      {
    ...
    syslog(LOG_INFO, "SV6621: OPEN_DEV ok (STA)\n");
  }
...
static int sv6621_scan_request(void)
{
  /* sv6621_scan_param (32-byte fixed head) + 2.4 GHz (ch 1-13, band 0) and
   * 5 GHz non-DFS channels (band 1) in the variable tail.
   * sv6621_scan_chan_info = {chan_num, band, scan_flags} = 3 bytes each.
   */
  ...
  g_sv6621_scan_n = 0;
  g_sv6621_scan_done = false;
  g_sv6621_scanning = true;

  ret = sv6621_send_cmd(SKW_CMD_START_SCAN, sp, sizeof(sp), NULL, NULL);
  if (ret < 0)
    {
      g_sv6621_scanning = false;
  ...
  for (wait = 0; wait < 500 && !g_sv6621_scan_done; wait++)
    {
      up_mdelay(10);
    }

  g_sv6621_scanning = false;
  return g_sv6621_scan_n;
}

/****************************************************************************
 * Name: sv6621_nv_patch
 *
 * Description:
...
static uint8_t *sv6621_nv_patch(const uint8_t *iram, int iram_len)
{
  const uint8_t *nv = g_sv6621_config->nv;
  uint32_t off;
  uint32_t size;
  uint8_t *copy;
  int i;

  if (nv == NULL || g_sv6621_config->nv_len < 36)
    {
      return NULL;
    }
  ...
  if (off == 0 || size == 0 || (int)(off + size) > g_sv6621_config->nv_len)
    {
      return NULL;
    }
...
static int sv6621_download_and_boot(void)
{
  const uint8_t *iram = g_sv6621_config->iram;
  int iram_len = g_sv6621_config->iram_len;
      ...
      if (memcmp(iram + off, "kees0616", 8) == 0)
        {
          ...
          iram_addr =
              p[0] | (p[1] << 8) | (p[2] << 16) | ((uint32_t)p[3] << 24);
          dram_addr =
              p[4] | (p[5] << 8) | (p[6] << 16) | ((uint32_t)p[7] << 24);
          break;
        }
    }
  ...
  sv6621_cmd52(true, 0, SKW_REG_DMA_TYPE, 0x01, NULL); /* ADMA */
  sv6621_cmd52(true, 0, SKW_REG_SLP, 0x01, NULL);      /* sleep disabled */

  /* Un-throttle every RX channel: the CP holds data for a port until its
   * flow-control bit is clear.
   */

  sv6621_cmd52(true, 0, SKW_REG_RX_FTL0, 0x00, NULL);
  sv6621_cmd52(true, 0, SKW_REG_RX_FTL1, 0x00, NULL);

  ret = sv6621_dt_stream(dram_addr, g_sv6621_config->dram,
                         g_sv6621_config->dram_len);
  if (ret < 0)
    {
      return ret;
    }
  ...
  {
    uint8_t *patched = sv6621_nv_patch(iram, iram_len);

    ret = sv6621_dt_stream(iram_addr, patched != NULL ? patched : iram,
                           iram_len);
    if (patched != NULL)
      {
        kmm_free(patched);
  ...
  sv6621_cmd52(true, 0, SKW_REG_DL_DONE, 0x01, NULL); /* boot the CP */
  return OK;
}
 ...
 *
 * Description:
 *   Ask the platform transport to enumerate and configure the SDIO link,
 *   verify the CP identity, then download and start the firmware.
 *
 ****************************************************************************/
...
static int sv6621_bringup(void)
{
  ...
  ret = transport->ops->initialize(transport, g_sv6621_config->power,
                                   g_sv6621_config->power_arg);
  if (ret < 0)
    {
      return ret;
    }

  sv6621_dt_latch(SKW_CP_CHIPID);
  memset(identity, 0, sizeof(identity));
  ret = sv6621_cmd53_read(1, SKW_DT_WINDOW, true, identity, sizeof(identity));
  if (ret < 0)
    {
      return ret;
    }

  if (memcmp(identity, "SV6160LITE", 10) != 0)
    {
      wlerr("ERROR: SV6621 CP identity mismatch\n");
      return -ENODEV;
    }

  wlinfo("SV6160LITE detected, downloading firmware\n");
  return sv6621_download_and_boot();
}
/****************************************************************************
 * Name: sv6621_wait_conn_evt
 *
 * Description:
...
static int sv6621_wait_conn_evt(int timeout_ms)
{
  ...
  for (t = 0; t < timeout_ms / 10; t++)
    {
      if (g_sv6621_conn_evt >= 0)
        {
          return g_sv6621_conn_evt;
        }
...
static bool sv6621_bss_is_rsn(const struct sv6621_bss_entry_s *bss)
{
  int off;
...
static int sv6621_associate(const struct sv6621_bss_entry_s *bss)
{
  uint8_t buf[320];
  int n;
  int evt;
...

  {
    ...
    sv6621_send_cmd(SKW_CMD_SET_MIB, mib, sizeof(mib), NULL, NULL);
  }

  /* JOIN: sv6621_join_param + the beacon IEs in the tail.  Field offsets:
  ...
  memcpy(g_sv6621_bssid, bss->bssid, 6);
...

  {
    ...
    ret = sv6621_send_cmd(SKW_CMD_JOIN, buf, n, jresp, &jrlen);
    if (ret < 0)
      {
    ...
    /* JOIN ACK returns sv6621_join_resp {peer_idx,lmac_id,inst,mcast_idx}. */

    g_sv6621_peer_idx = (jrlen > 0) ? jresp[0] : 0;
    g_sv6621_lmac = (jrlen > 1) ? (jresp[1] & 0x3) : 0;
    g_sv6621_mcast = (jrlen > 3) ? jresp[3] : 0;
    syslog(LOG_ERR,
           "SV6621: JOIN resp len=%d %02x %02x %02x %02x "
           "peer_idx=%d lmac=%d\n",
           jrlen, jrlen > 0 ? jresp[0] : 0, jrlen > 1 ? jresp[1] : 0,
           jrlen > 2 ? jresp[2] : 0, jrlen > 3 ? jresp[3] : 0,
           g_sv6621_peer_idx, g_sv6621_lmac);
  }
  ...
  ret = sv6621_send_cmd(SKW_CMD_AUTH, buf, 14, NULL, NULL);
  if (ret < 0)
    {
  ...
  memset(buf, 0, sizeof(buf));
  {
    ...
    static const uint8_t htcap[26] = {
      ...
      0x00, 0x00, 0x00, 0x00, 0x00,      /* rx_highest + tx_params */
  ...
  ret = sv6621_send_cmd(SKW_CMD_ASSOC, buf, n, NULL, NULL);
  if (ret < 0)
    {
  ...
  evt = sv6621_wait_conn_evt(3000);
  syslog(LOG_INFO, "SV6621: connect result = %d (%s)\n", evt,
         evt == SKW_CONN_ASSOC_OK     ? "ASSOCIATED"
         : evt == SKW_CONN_ASSOC_FAIL ? "rejected"
                                      : "timeout");
...
int sv6621_initialize(const struct sv6621_config_s *config)
{
  int ret;

  if (config == NULL || config->transport == NULL ||
      config->transport->ops == NULL ||
      config->transport->ops->initialize == NULL ||
      config->transport->ops->readb == NULL ||
      config->transport->ops->writeb == NULL ||
      config->transport->ops->read == NULL ||
      config->transport->ops->write == NULL ||
      config->transport->ops->ack_interrupt == NULL || config->power == NULL ||
      config->iram == NULL || config->dram == NULL)
    {
      return -EINVAL;
    }
  ...
  if (g_sv6621_run)
    {
      return (g_sv6621_service & SV6621_STATE_WIFI) ? OK : -EBUSY;
    }

  g_sv6621_config = config;
  ...
  nxsem_init(&g_sv6621_cmd.done, 0, 0);

  g_sv6621_run = true;
  ret = kthread_create("sv6621_rx", CONFIG_SV6621_RXPRIO,
                       CONFIG_SV6621_RXSTACK, sv6621_rx_thread, NULL);
  if (ret < 0)
    {
      g_sv6621_run = false;
  ...
  for (ret = 0; ret < 200; ret++)
    {
      if (g_sv6621_service & SV6621_STATE_WIFI)
        {
          break;
        }
...
#ifdef CONFIG_NET
  ret = sv6621_netdev_register();
#endif

  return ret;
}

uint32_t sv6621_state(void) { return g_sv6621_service; }

int sv6621_scan(struct sv6621_bss_s *list, int max)
{
  int n;
  int i;
  ...
  if (list != NULL)
    {
      ...
      for (i = 0; i < cnt; i++)
        {
          memcpy(list[i].bssid, g_sv6621_scan[i].bssid, 6);
          memcpy(list[i].ssid, g_sv6621_scan[i].ssid, 33);
          list[i].ssid_len = g_sv6621_scan[i].ssid_len;
          list[i].channel = g_sv6621_scan[i].channel;
          list[i].band = g_sv6621_scan[i].band;
          list[i].rssi = g_sv6621_scan[i].rssi;
        }
    }
...
int sv6621_connect(const char *ssid)
{
  int i;
  int pass;

  if (!(g_sv6621_service & SV6621_STATE_WIFI) || ssid == NULL)
    {
      return -ENODEV;
    }
  ...
  for (pass = 0; pass < 2; pass++)
    {
      for (i = 0; i < g_sv6621_scan_n; i++)
        {
          if (g_sv6621_scan[i].ssid_len == (int)strlen(ssid) &&
              memcmp(g_sv6621_scan[i].ssid, ssid, g_sv6621_scan[i].ssid_len) ==
                  0)
            {
              return sv6621_associate(&g_sv6621_scan[i]);
            }
        }
...
/****************************************************************************
 * Name: sv6621_get_mac / sv6621_get_bssid
 ****************************************************************************/

void sv6621_get_mac(uint8_t mac[6]) { memcpy(mac, g_sv6621_mac, 6); }

void sv6621_get_bssid(uint8_t bssid[6]) { memcpy(bssid, g_sv6621_bssid, 6); }

/****************************************************************************
 * Name: sv6621_send_control
 ****************************************************************************/

int sv6621_send_control(uint8_t id, const uint8_t *payload, int length)
{
  return sv6621_send_cmd(id, payload, length, NULL, NULL);
}
 ...
 *   channel-7 packet: outer header + sv6621_tx_desc_hdr(6) +
 *sv6621_tx_desc_conf (2) + Ethernet frame + trailing eof terminator.
 *Best-effort TX credit handling (read 0x184, write consumed to 0x168); the CP
 *ignores credit when SKW_FLAG_FW_IGNORE_CRED is set.
 *
 ****************************************************************************/

int sv6621_data_tx(const uint8_t *eth, int ethlen)
{
  ...
  if (ethlen < 14 || padded > (int)sizeof(g_sv6621_txbuf))
    {
      return -E2BIG;
    }
  ...
  sv6621_cmd52(false, 0, 0x184, 0, &credit);

  memset(g_sv6621_txbuf, 0, padded);

  hdr = SKW_HDR(SKW_CH_WIFI_DATA, inner);
  g_sv6621_txbuf[0] = hdr & 0xff;
  g_sv6621_txbuf[1] = (hdr >> 8) & 0xff;
  g_sv6621_txbuf[2] = (hdr >> 16) & 0xff;
  g_sv6621_txbuf[3] = (hdr >> 24) & 0xff;

  /* sv6621_tx_desc_hdr word0: padding_gap:2 inst:2 tid:4 peer_lut:5
   * frame_type:1 encry_dis:1 rate:1.  STA data: peer_lut = peer_idx,
   * everything else 0 (frame_type SKW_ETHER_FRAME, encry enabled).
   */

  {
    uint8_t lut = (eth[0] & 0x01) ? g_sv6621_mcast : g_sv6621_peer_idx;
  ...
  g_sv6621_txbuf[4] = w0 & 0xff;
  g_sv6621_txbuf[5] = (w0 >> 8) & 0xff;

  /* word1: msdu_len:12 lmac_id:2 rsv:2 */

  w1 = (ethlen & 0x0fff) | ((uint16_t)(g_sv6621_lmac & 0x3) << 12);
  g_sv6621_txbuf[6] = w1 & 0xff;
  g_sv6621_txbuf[7] = (w1 >> 8) & 0xff;
  ...
  g_sv6621_txbuf[8] = eth[12];
  g_sv6621_txbuf[9] = eth[13];

  /* sv6621_tx_desc_conf (2B): checksum offload off. */

  g_sv6621_txbuf[10] = 0;
  g_sv6621_txbuf[11] = 0;

  memcpy(g_sv6621_txbuf + 12, eth, ethlen);
    ...
    if (filter)
      {
...

        {
          ...
          memcpy(eapbuf, g_sv6621_txbuf + 4, cp);
          nxmutex_unlock(&g_sv6621_tx_lock);
          return sv6621_send_cmd(SKW_CMD_TX_DATA_FRAME, eapbuf, cp, NULL,
                                 NULL);
        }
      }
  }
  ...
  eof = SKW_HDR_EOF_TERM;
  g_sv6621_txbuf[4 + inner + 0] = eof & 0xff;
  g_sv6621_txbuf[4 + inner + 1] = (eof >> 8) & 0xff;
  g_sv6621_txbuf[4 + inner + 2] = (eof >> 16) & 0xff;
  g_sv6621_txbuf[4 + inner + 3] = (eof >> 24) & 0xff;

  /* Return one consumed credit to the CP (best-effort). */

  if (credit > 0)
    {
      sv6621_cmd52(true, 0, 0x168, 1, NULL);
    }

  ret = sv6621_cmd53_write(1, SKW_PK_WINDOW, false, g_sv6621_txbuf, padded);
 ...
 *
 * Description:
 *   Install a key into the CP via ADD_KEY(12).  Builds sv6621_key_params:
                   ...
                   const uint8_t *pn)
{
  ...
  return sv6621_send_cmd(SKW_CMD_ADD_KEY, buf, n, NULL, NULL);
}
