...
static int rk3576_mailbox_interrupt(int irq, void *context, void *arg)
{
  uint32_t status = mbox_getreg(g_mbox_rx_base, RK3576_MBOX_A2B_STATUS);
  uint32_t cmd;
  uint32_t data;
  ...
  if ((status & RK3576_MBOX_INT_MASK) != 0)
    {
      ...
      mbox_putreg(RK3576_MBOX_INT_MASK, g_mbox_rx_base,
...
int rk3576_mailbox_send(unsigned int instance, uint32_t cmd, uint32_t data)
{
  ...
  mbox_putreg(RK3576_MBOX_TRIGGER_UPDATE | RK3576_MBOX_TRIGGER_MASK, base,
              RK3576_MBOX_B2A_INTEN);

  if ((mbox_getreg(base, RK3576_MBOX_B2A_STATUS) & RK3576_MBOX_INT_MASK) != 0)
    {
      return -EBUSY;
    }
