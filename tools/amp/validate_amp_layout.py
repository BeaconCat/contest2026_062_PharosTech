...
def parse_dtsi(path: Path) -> dict[str, tuple[int, int]]:
    ...
    for cpu in range(4):
        require(rf'&cpu_l{cpu}\s*\{{\s*status\s*=\s*"disabled";', text, path)
...
def validate_config(path: Path) -> None:
    ...
    for key in ("CONFIG_UP", "CONFIG_RK3576_DMA_ALLOC"):
        if values.get(key) == "y":
...
def validate_dtb(path: Path) -> None:
    ...
    expected = {cpu: "disabled" for cpu in range(4)}
    expected.update({0x100 + cpu: "okay" for cpu in range(4)})
