...
"$kernel/scripts/kconfig/merge_config.sh" -m -O "$output" \
  "$output/.config" "$kernel/arch/arm64/configs/rockchip_amp.config" \
  "$script_dir/nyabula_amp.fragment"
make -C "$kernel" O="$output" ARCH=arm64 \
  CROSS_COMPILE="$cross_compile" olddefconfig

for option in ROCKCHIP_AMP ROCKCHIP_MBOX RPMSG_ROCKCHIP_MBOX \
              RPMSG_CHAR RPMSG_CTRL ROCKCHIP_RKNPU BLK_DEV_INITRD; do
