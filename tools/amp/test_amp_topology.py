...
class TopologyTest(unittest.TestCase):
    ...
    def dtb(self, changes=None, missing=None, address_cells=2):
        ...
        cpus = {cpu: "disabled" for cpu in range(4)}
        cpus.update({0x100 + cpu: "okay" for cpu in range(4)})
    ...
    def fit(self, config, dtb, firmware=b"test"):
        if not shutil.which("mkimage"):
            self.skipTest("mkimage required")
        kernel = self.root / "Image"
        kernel.write_bytes(bytes(56) + b"ARM\x64" + bytes(4))
