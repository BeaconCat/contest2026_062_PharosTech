...
def pack_plugin(project: Path, output: Path | None) -> Path:
    manifest = validate_manifest(project / "manifest.json")
...
def verify_archive(path: Path) -> tuple[dict[str, object], dict[str, bytes]]:
    files: dict[str, bytes] = {}
    try:
    ...
    expected_names = set(files) - {"hashes.json"}
...
def install_archive(archive: Path, root: Path) -> Path:
    manifest, files = verify_archive(archive)
...
def main() -> int:
    ...
    try:
        if args.command == "build":
            ...
            print(pack_plugin(args.project.resolve(), args.output))
        elif args.command == "verify":
            manifest, _ = verify_archive(args.archive.resolve())
            print(f"PASS {manifest['id']} {manifest['version']}")
        elif args.command == "install":
            print(install_archive(args.archive.resolve(), args.root.resolve()))
        return 0
