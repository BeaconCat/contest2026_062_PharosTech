...
static void test_sbc_damage(void)
{
  ...
  assert(ny_sbc_decode(decoder, g_vector_sbc, SBC_FRAME_BYTES, &used, pcm,
                       100, NULL) == -ENOSPC);
  assert(used == 0);

  /* A frame cut short waits for the rest. */

  assert(ny_sbc_decode(decoder, g_vector_sbc, SBC_FRAME_BYTES - 1, &used,
                       pcm, 256, NULL) == -EAGAIN);
  assert(used == 0);
  ...
  assert(ny_sbc_decode(decoder, damaged, sizeof(damaged), &used, pcm, 256,
                       NULL) == -EBADMSG);
  assert(used == SBC_FRAME_BYTES);
  assert(ny_sbc_decode(decoder, damaged + used, sizeof(damaged) - used,
                       &used, pcm, 256, NULL) == 128);
  assert(used == SBC_FRAME_BYTES);

  /* Bytes that hold no frame at all are given up on, never looped over. */

  memset(noise, 0x55, sizeof(noise));
  assert(ny_sbc_decode(decoder, noise, sizeof(noise), &used, pcm, 256,
                       NULL) < 0);
...
static void test_msbc_decode(void)
{
  ...
  assert(tone_share(all + NY_MSBC_FRAME_SAMPLES, 3 * NY_MSBC_FRAME_SAMPLES,
                    1, 16000, 1000) > 0.95);
  ...
  for (count = 0; count < 8; count++)
    {
      ...
      assert(ny_sbc_decode(decoder, frame, sizeof(frame), &used, pcm,
                           NY_SBC_FRAME_SAMPLES_MAX, NULL) ==
             NY_MSBC_FRAME_SAMPLES);
      memcpy(out + count * NY_MSBC_FRAME_SAMPLES, pcm,
             NY_MSBC_FRAME_SAMPLES * sizeof(int16_t));
    }

  assert(tone_share(out + 2 * NY_MSBC_FRAME_SAMPLES,
                    6 * NY_MSBC_FRAME_SAMPLES, 1, NY_MSBC_RATE, 440) > 0.9);
...
static void test_h2_framing(void)
{
  ...
  memmove(stream + 3 + 5 * NY_MSBC_H2_BYTES,
          stream + 3 + 6 * NY_MSBC_H2_BYTES, 3 * NY_MSBC_H2_BYTES);
