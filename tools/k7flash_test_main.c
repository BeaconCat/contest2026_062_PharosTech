...
extern int kickpi_k7_wifi_ap_test_start(int secure);
extern int kickpi_k7_wifi_ap_test_stop(void);
extern int kickpi_k7_wifi_ap_test_status(void);
extern int kickpi_k7_wifi_roam_test(void);
extern int kickpi_k7_wifi_prepare_sleep(void);
extern int kickpi_k7_wifi_abort_sleep(void);
extern int kickpi_k7_wifi_host_wake_status(void);
extern int kickpi_k7_wifi_sleep_switch(int write, int value);
...
int main(int argc, char *argv[])
{
  ...
  if (argc >= 3 && strcmp(argv[1], "aptest") == 0)
    {
      if (strcmp(argv[2], "start") == 0)
        {
          int security = argc > 3 && strcmp(argv[3], "wpa3") == 0 ? 2 :
                         argc > 3 && strcmp(argv[3], "transition") == 0 ? 3 :
                         1;

          ret = kickpi_k7_wifi_ap_test_start(security);
        }
      else if (strcmp(argv[2], "stop") == 0)
        {
          ret = kickpi_k7_wifi_ap_test_stop();
        }
      else if (strcmp(argv[2], "status") == 0)
        {
          ret = kickpi_k7_wifi_ap_test_status();
        }
      else
        {
          return 1;
      ...
      printf("aptest: %s returned %d\n", argv[2], ret);
      return ret < 0 ? 1 : 0;
    }

  if (argc == 3 && strcmp(argv[1], "pmtest") == 0)
    {
      ret = strcmp(argv[2], "suspend") == 0 ?
          kickpi_k7_wifi_prepare_sleep() :
          strcmp(argv[2], "resume") == 0 ?
          kickpi_k7_wifi_abort_sleep() : -EINVAL;
      printf("pmtest: %s returned %d\n", argv[2], ret);
      return ret < 0 ? 1 : 0;
    }

  if (argc == 2 && strcmp(argv[1], "wakecheck") == 0)
    {
      ret = kickpi_k7_wifi_host_wake_status();
      printf("wakecheck: count=%d level=%d\n", ret / 10, ret % 10);
      return 0;
    }

  if (argc == 2 && strcmp(argv[1], "rxcount") == 0)
    {
      extern int kickpi_k7_wifi_data_rx_count(void);

      ret = kickpi_k7_wifi_data_rx_count();
      printf("rxcount: %d\n", ret);
      return ret < 0 ? 1 : 0;
    }

  if (argc == 2 && strcmp(argv[1], "roamtest") == 0)
    {
      ret = kickpi_k7_wifi_roam_test();
      printf("roamtest: %d\n", ret);
      return ret < 0 ? 1 : 0;
    }

  if ((argc == 2 || argc == 3) && strcmp(argv[1], "slpswitch") == 0)
    {
      ret = kickpi_k7_wifi_sleep_switch(argc == 3,
