              ...
              if (ret < 0)
                {
                  ...
                  worker->playing = false;
                  worker->retry_at =
                      ny_bt_audio_now() + NY_BT_DEVICE_RETRY_MS;
                  nxmutex_lock(&g_bt_audio.lock);
                  g_bt_audio.device_closed = true;
                  g_bt_audio.status.owner = NY_BT_AUDIO_OWNER_NONE;
                  nxmutex_unlock(&g_bt_audio.lock);
  ...
  if (now - worker->last_rx > NY_BT_CALL_SILENT_RX_MS)
    {
      ...
      size_t per_second = worker->msbc_call
                              ? NY_MSBC_H2_BYTES * 1000 * 10 / 75
                              : 16000;
  ...
  for (; ; )
    {
      ...
      if (desired != worker.mode ||
          (desired == NY_BT_AUDIO_OWNER_CALL &&
          ...
          if (desired == NY_BT_AUDIO_OWNER_CALL)
            {
              ...
              ny_pcm_gate_init(&worker.gate,
                               CONFIG_NYABULA_CORE_BT_GATE_THRESHOLD,
                               CONFIG_NYABULA_CORE_BT_GATE_ATTENUATION_DB,
                               CONFIG_NYABULA_CORE_BT_GATE_HANG_MS,
                               msbc ? 75 : 30);
            }
        }
                             ...
                             uint8_t packet_status)
{
  ...
  if (g_bt_audio.want_call)
    {
      queued = ny_pcm_ring_put(&g_bt_audio.sco_ring, record,
                               (uint16_t)(size + 1));
    }
