...
#define NY_BT_WORK_AVRCP_ANY      (NY_BT_WORK_AVRCP_CAPS | \
                                   NY_BT_WORK_AVRCP_PLAYBACK | \
                                   NY_BT_WORK_AVRCP_TRACK | \
                                   NY_BT_WORK_AVRCP_ATTRS | \
                                   NY_BT_WORK_AVRCP_STATUS | \
                                   NY_BT_WORK_AVRCP_RELEASE | \
...
static void ny_bt_a2dp_recv(struct bt_a2dp_stream *stream,
                            struct net_buf *buf, uint16_t sequence,
                            uint32_t timestamp);
...
static void ny_bt_ct_passthrough_rsp(struct bt_avrcp_ct *ct, uint8_t tid,
                                     bt_avrcp_rsp_t result,
                                     const struct bt_avrcp_passthrough_rsp
                                         *rsp);
...
static void ny_bt_hf_sco_connected(struct bt_hfp_hf *hf,
                                   struct bt_conn *sco);
...
static const char *const g_bt_state_names[] =
{
  "off", "starting", "ready", "failed"
};

static const char *const g_bt_a2dp_names[] =
{
  "disconnected", "connected", "open", "streaming"
};

static const char *const g_bt_call_names[] =
{
  "idle", "incoming", "dialing", "alerting", "active", "held"
};
  ...
  cJSON_ArrayForEach(row, cJSON_GetObjectItemCaseSensitive(root, "devices"))
    {
      ...
      const cJSON *last =
          cJSON_GetObjectItemCaseSensitive(row, "lastConnected");
      struct ny_bt_device_s *device;
      bt_addr_t parsed;
...
static void ny_bt_tick_avrcp(uint32_t work)
{
  ...
  if (work & NY_BT_WORK_AVRCP_RELEASE)
  ...
  else if (work & NY_BT_WORK_AVRCP_CAPS)
    {
      ...
      ret = bt_avrcp_ct_get_caps(ct, ny_bt_tid(),
  ...
  else if (work & NY_BT_WORK_AVRCP_TRACK)
    {
      ...
      ret = bt_avrcp_ct_register_notification(ct, ny_bt_tid(),
...
static void ny_bt_tick_volume(void)
{
  ...
  nxmutex_unlock(&g_bt.lock);
  if (pending >= 0 &&
      ny_product_audio_set_levels(pending, pending > 0 ? 0
...
static cJSON *ny_bt_status(void)
{
  ...
  nxmutex_lock(&g_bt.lock);
  ok = ok && cJSON_AddStringToObject(root, "state",
                                     g_bt_state_names[g_bt.state]);
  ...
  ok = ok && cJSON_AddBoolToObject(root, "connectable",
                                   g_bt.state == NY_BT_READY);
  ...
  if (g_bt.pairing != NULL)
    {
      ...
      ok = ok && pairing != NULL &&
           cJSON_AddStringToObject(pairing, "address",
                                   g_bt.pairing_address) &&
  ...
  ok = ok && cJSON_AddNumberToObject(avrcp, "volume",
                                     g_bt.volume_seen < 0 ? 0
                                                          : g_bt.volume_seen);
  ok = ok && cJSON_AddBoolToObject(avrcp, "absoluteVolume",
                                   g_bt.volume_absolute >= 0);

  ok = ok && cJSON_AddStringToObject(hfp, "state",
                                     g_bt.hf != NULL ? "connected"
                                                     : "disconnected");
  ok = ok && cJSON_AddStringToObject(hfp, "call",
                                     g_bt_call_names[g_bt.call_state]);
  if (g_bt.number[0] != '\0')
    {
  ...
  ok = ok && cJSON_AddStringToObject(
                 hfp, "codec",
                 !g_bt.sco_up                              ? "none"
                 : g_bt.codec_id == BT_HFP_HF_CODEC_MSBC ? "mSBC"
                                                           : "CVSD");
  ...
  ok = ok && cJSON_AddNumberToObject(sound, "bufferMs",
                                     audio.media_buffer_ms);
  ok = ok && cJSON_AddNumberToObject(sound, "mediaPackets",
                                     audio.media_packets);
  ok = ok && cJSON_AddNumberToObject(sound, "mediaFrames",
                                     audio.media_frames);
  ok = ok && cJSON_AddNumberToObject(sound, "mediaBadFrames",
                                     audio.media_bad);
  ok = ok && cJSON_AddNumberToObject(sound, "mediaDropped",
                                     audio.media_dropped);
  ok = ok && cJSON_AddNumberToObject(sound, "mediaUnderruns",
                                     audio.media_underruns);
  ok = ok && cJSON_AddNumberToObject(sound, "scoRxPackets",
                                     audio.sco_rx_packets);
  ok = ok && cJSON_AddNumberToObject(sound, "scoRxBad", audio.sco_rx_bad);
  ok = ok && cJSON_AddNumberToObject(sound, "scoTxPackets",
                                     audio.sco_tx_packets);
  ok = ok && cJSON_AddNumberToObject(sound, "scoTxFailed",
                                     audio.sco_tx_failed);
  ok = ok && cJSON_AddBoolToObject(sound, "micGated", audio.gate_closed);
  ok = ok && cJSON_AddNumberToObject(sound, "micGateClosures",
                                     audio.gate_closures);
  if (!ok)
    {
      cJSON_Delete(root);
                          ...
                          cJSON **result)
{
  ...
  if (strcmp(topic, "bt.discoverable") == 0)
  ...
  else if (strcmp(topic, "bt.connect") == 0)
    {
      ...
      nxmutex_lock(&g_bt.lock);
      ret = g_bt.acl != NULL ? (strcasecmp(text, g_bt.peer_address) == 0
                                    ? -EALREADY
                                    : -EBUSY)
                             : 0;
      nxmutex_unlock(&g_bt.lock);
  ...
  else if (strcmp(topic, "bt.call.hangup") == 0)
    {
      ret = call == NULL                         ? -ENOENT
            : call_state == NY_BT_CALL_INCOMING ? Z_API(bt_hfp_hf_reject)(call)
                                                : Z_API(bt_hfp_hf_terminate)(
                                                      call);
    }
  else
    {
...
int ny_product_bt_tick(void)
{
  ...
  save = g_bt.devices_dirty && g_bt.devices_loaded &&
         now >= g_bt.devices_retry;
  nxmutex_unlock(&g_bt.lock);
